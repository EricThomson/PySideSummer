# -*- coding: utf-8 -*-
"""
mymoviesPyside_ans.py
A solution to Exercise at end of Chapter 8
...
import resource_rc
import addeditmoviedlgPyside_ans
...
class MainWindow(QtGui.QMainWindow):
    def __init__(self, parent=None):
        ...
        self.movies = moviedataPyside_ans.MovieContainer() 
    ...
    def editAdd(self):
        ...
        form = addeditmoviedlgPyside_ans.AddEditMovieDlg(self.movies, None, self)
        #print dir(form)
        if form.exec_():
    ...
    def editEdit(self):
        ...
        if movie is not None:
            form = addeditmoviedlgPyside_ans.AddEditMovieDlg(self.movies, movie, self)
            if form.exec_():
    ...
    def updateTable(self, current=None):
        ...
        for row, movie in enumerate(self.movies):
            ...
            #Date acquired
            item = QtGui.QTableWidgetItem(movie.acquired.toString(moviedataPyside_ans.DATEFORMAT))
            item.setTextAlignment(QtCore.Qt.AlignRight | QtCore.Qt.AlignVCenter)
            ...
            if movie.location:
                ...
                self.table.setItem(row, 4, QtGui.QTableWidgetItem(location))
            #Notes
            notes = movie.notes
            if len(notes) > 60:
                notes = notes[:59] + "..."
            self.table.setItem(row, 5, QtGui.QTableWidgetItem(notes))
